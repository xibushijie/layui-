layui.config({
    ...
    //定时任务列表数据
    var url = esbUrl + '/ktJob/getAllKtJob';
    dataTable(url);

    // $.ajax({
    //     type: "post",
    //     url: esbUrl + '/ktJob/getAllKtJob',
    //     dataType: "josn",
    //     success: function (response) {
    //         console.log(response+"定时")
    //     }
    // });
    ...
    function dataTable(url, where) {
        table.render({
            ...
            limit: 10,
            limits: [10, 15, 20, 25],
            id: "timedTaskListTable",
            cols: [[
                { type: "checkbox", fixed: "left", width: 50 },
                { field: 'ktJobCreattime', title: '创建时间', align: 'center' },
                { field: 'ktJobCreatuser', title: '创建用户', align: 'center' },
                { field: 'ktJobDbFile', title: '数据库文件', align: 'center' },
                { field: 'ktJobDescribe', title: '任务描述', align: 'center' },
                { field: 'ktJobExecute', title: '执行周期', align: 'center' },
                { field: 'ktJobName', title: '文件名', align: 'center' },
                { field: 'ktJobUrl', title: '文件路径', align: 'center' },
                { field: 'ktJobStatus', title: '任务状态', align: 'center' },
                { title: '操作', width: 170, templet: '#timedTaskListBar', fixed: "right", align: "center" }
            ]],
            done: function(res){
                console.log(res + "定时任务");
            }
        });
    }
    ...
    $(".addNews_btn").click(function () {
        layui.layer.open({
            ...
            success: function () {
                ...
                $("input[name='mechanismId']").val(mechanismId);
    ...
    $(".delAll_btn").click(function () {
        var checkStatus = table.checkStatus('timedTaskListTable'),
    ...
    //列表操作
    table.on('tool(timedTaskList)', function (obj) {
