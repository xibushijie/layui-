layui.config({
    ...
    $(".links").click(function () {
        parent.layui.element.tabAdd('bodyTab', {
            title: "新标签" + "<i class='layui-icon layui-unselect layui-tab-close'>ဆ</i>",
            content: '<iframe name="iframe" src="page/Safety/Add.html" frameborder="0" style="width: 100%;"></iframe>',
        });
    })
    ...
    function dataTable(url, where) {
        table.render({
            ...
            cols: [[
                ...
                { field: 'applicationId', title: 'id' },
                { field: 'applicationName', title: 'ESB应用名称' },
                { field: 'applicationParent', title: 'ESB应用父级id' },
                { field: 'imgURL', title: 'ESB应用图片地址' },
                { title: '操作', width: 170, templet: '#esbListBar', fixed: "right", align: "center" }
            ]],
            done: function (res) {
                console.log(res)
            }
