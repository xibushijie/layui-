...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		//显示左侧菜单 
		var _this = this;
		// $(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="../platForm/menu.html"><i class="layui-icon" data-icon=""></i><cite>菜单管理</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
		$(".navBar ul").html('').append(_this.navBar(dataStr)).height($(window).height()-210);
		$(".navBar ul li").eq(0).addClass("layui-this");
	...
	Tab.prototype.changeRegresh = function(index){
        if(changeRefreshStr == "true"){
            $(".clildFrame .layui-tab-item").eq(index).find("iframe")[0].contentWindow.location.reload();//点击tab zhaodao 
        }
	}
